...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uKHC
                        ...
                        bool* pfMissingInputs)
{
    ...
    if (fCheckInputs)
    {
        ...
        if (fLimitFree && nFees < CTransaction::nMinRelayTxFee)
        {
            ...
            dFreeCount *= pow(1.0 - 1.0/420.0, (double)(nNow - nLastTime));
            nLastTime = nNow;
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 50000 blocks, which will occur approximately every 250 days
...
unsigned int ComputeMinWork(unsigned int nBase, int64 nTime)
{
    ...
    while (nTime > 0 && bnResult < bnProofOfWorkLimit)
    {
        ...
        bnResult *= 4;
        // ... in best-case exactly -times-normal target time
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1389356375, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0x35a10d9a << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0x35, 0xa1, 0x0d, 0x9a };
